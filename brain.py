...
async def detect_language_ai(text: str) -> tuple[str | None, float]:
    ...
    try:
        ...
        if prediction and prediction[0] and prediction[1]:
            ...
            logger.debug(f"Detected language: {lang_code} (Conf: {confidence:.2f}) for text: '{clean_text[:50]}...'")
...
async def log_moderation_action(
    ...
    embed.add_field(name="Moderator", value=f"{moderator.mention} (`{moderator.id}`)" if moderator else "Automated Action", inline=True)
    if message_url:
...
async def check_openai_moderation_api(text_content: str) -> dict:
    if not text_content.strip() or not http_session or http_session.closed:
...
async def apply_moderation_punishment(
    ...
    extra_log_fields, duration = [], None
    ...
    if not guild.me or guild.me.top_role <= member.top_role:
        logger.warning(f"Cannot {action_type} {member.display_name}: Check bot role hierarchy and permissions.")
        return

    try:
        try:
            if dm_message_text: await member.send(dm_message_text)
        ...
        elif action_type == "temp_ban" and duration:
            if not db_conn: return
            unban_time = datetime.now(timezone.utc) + duration
            async with db_conn.cursor() as cursor:
                await cursor.execute('INSERT OR REPLACE INTO temp_bans (user_id, guild_id, unban_time, ban_reason) VALUES (?, ?, ?, ?)',
...
async def process_infractions_and_punish(
    member: discord.Member, guild: discord.Guild, violation_types: list[str],
    content: str, message_url: str | None
):
    if not db_conn: return
    ...
    async with db_conn.cursor() as cursor:
        ...
        await cursor.execute("SELECT SUM(points) FROM infractions WHERE user_id = ? AND guild_id = ? AND timestamp >= ?",
...
async def get_user_infractions_from_db(user_id: int, guild_id: int, days_limit: int = 90) -> tuple[list[dict], int]:
    ...
    try:
        ...
        async with db_conn.execute(
            "SELECT id, violation_type, points, timestamp FROM infractions WHERE user_id = ? AND guild_id = ? AND timestamp >= ? ORDER BY timestamp DESC",
            (user_id, guild_id, past_time_limit)
        ) as cursor:
            infractions = [{"id": r[0], "violation_type": r[1], "points": r[2], "timestamp": r[3]} for r in await cursor.fetchall()]

        ninety_days_ago = (datetime.now(timezone.utc) - timedelta(days=90)).isoformat()
        async with db_conn.execute("SELECT SUM(points) FROM infractions WHERE user_id = ? AND guild_id = ? AND timestamp >= ?",
                                   (user_id, guild_id, ninety_days_ago)) as cursor:
            result = await cursor.fetchone()
...
async def remove_specific_infraction_from_db(infraction_id: int) -> bool:
    ...
    try:
        async with db_conn.cursor() as cursor:
            await cursor.execute("DELETE FROM infractions WHERE id = ?", (infraction_id,))
    ...
    try:
        ...
        await db_conn.execute("""
            CREATE TABLE IF NOT EXISTS guild_configs (
                guild_id INTEGER NOT NULL, config_key TEXT NOT NULL,
                config_value TEXT, PRIMARY KEY (guild_id, config_key)
            )""")
        await db_conn.execute("""
            CREATE TABLE IF NOT EXISTS infractions (
                id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, guild_id INTEGER NOT NULL,
                violation_type TEXT NOT NULL, points INTEGER NOT NULL, message_content_snippet TEXT,
                message_url TEXT, timestamp TEXT NOT NULL
            )""")
        ...
        await db_conn.execute("""
            CREATE TABLE IF NOT EXISTS temp_bans (
                ...
                unban_time TEXT NOT NULL, ban_reason TEXT,
                PRIMARY KEY (user_id, guild_id)
            )""")
        await db_conn.execute('CREATE INDEX IF NOT EXISTS idx_temp_bans_unban_time ON temp_bans (unban_time)')
        await db_conn.execute("""
            CREATE TABLE IF NOT EXISTS dynamic_rules (
                id INTEGER PRIMARY KEY, guild_id INTEGER NOT NULL, rule_type TEXT NOT NULL,
                pattern TEXT NOT NULL, added_by_id INTEGER NOT NULL, timestamp TEXT NOT NULL,
                UNIQUE (guild_id, rule_type, pattern)
            )""")
        await db_conn.execute("""
            CREATE TABLE IF NOT EXISTS review_queue (
                id INTEGER PRIMARY KEY, guild_id INTEGER NOT NULL, user_id INTEGER NOT NULL,
                channel_id INTEGER, message_id INTEGER UNIQUE, message_content TEXT NOT NULL,
    ...
    async def help_command(self, interaction: discord.Interaction):
        ...
        for cog_name, cog in self.bot.cogs.items():
            ...
            for cmd in cog.get_app_commands():
                desc = f"`/{cmd.name}` - {cmd.description}" if isinstance(cmd, app_commands.Command) else \
                       "\n".join([f"`/{cmd.name} {sub.name}` - {sub.description}" for sub in cmd.commands])
                command_list.append(desc)
            if command_list:
...
class ModerationCog(commands.Cog, name="Moderation"):
    ...
    async def get_effective_channel_language_config(self, guild_id: int, channel_id: int) -> list[str] | None:
        ...
        if channel_setting is not None: return channel_setting
    ...
    async def temp_ban_check_task(self):
        ...
        for ban in [b for b in expired_bans if now_utc >= b["unban_time"]]:
            ...
            try:
                ...
                logger.info(f"Unbanned user {ban['user_id']} from {guild.name} (temp ban expired).")
                await log_moderation_action("unban_temp_expired", user_obj, "Temporary ban automatically expired.", guild=guild, color=discord.Color.green())
    ...
    async def cleanup_old_infractions_task(self):
        ...
        try:
            async with db_conn.cursor() as cursor:
                await cursor.execute("DELETE FROM infractions WHERE timestamp < ?", (one_hundred_eighty_days_ago,))
                await db_conn.commit()
                if cursor.rowcount > 0:
    ...
    async def _handle_violations(self, message: discord.Message, violations: set[str]):
        logger.info(f"User {message.author.id} ({message.author.name}) triggered: {list(violations)}. URL: {message.jump_url}")
        ...
        if await get_guild_config(message.guild.id, "send_in_channel_warning", bot_config.send_in_channel_warning):
            ...
            warn_text = f"{message.author.mention}, your message was moderated: **{viol_summary}**. Please review server rules."
            try:
    ...
    def check_spam(self, user_id: int, guild_id: int, cleaned_content: str) -> set[str]:
        ...
        if cleaned_content and "spam_rate" not in violations:
            ...
            if len(history) >= bot_config.spam_repetition_history_count - 1 and \
               sum(1 for old in history if fuzz.ratio(cleaned_content, old) > bot_config.spam_repetition_fuzzy_threshold) >= bot_config.spam_repetition_history_count - 1:
    ...
    async def check_language(self, guild_id: int, channel_id: int, raw_content: str) -> set[str]:
        if len(raw_content.split()) < bot_config.min_msg_len_for_lang_check: return set()
        ...
        if confidence >= threshold and not any(word in raw_content.lower() for word in bot_config.common_safe_foreign_words):
            logger.info(f"Language violation: Detected '{lang_code}' ({confidence:.2f}) where {lang_config} required.")
    ...
    async def check_ai_text_moderation(self, content: str, guild_id: int) -> tuple[set[str], str | None]:
        ...
        try:
            ...
            if result.get("flagged"):
                severe = {"harassment", "harassment/threatening", "hate", "hate/threatening", "self-harm", "sexual/minors", "violence/graphic"}
                if any(result["categories"].get(cat) for cat in severe):
    ...
    async def check_ai_media_moderation(self, attachments: list[discord.Attachment], guild_id: int) -> set[str]:
        if not SIGHTENGINE_API_USER: return set()
        thresholds = { "nsfw_media": 0.45, "gore_violence_media": 0.5, "offensive_symbols_media": 0.45 }
        for attach in attachments:
            ...
            try:
                ...
                if result.get("nudity", {}).get("sexual_activity", 0) > thresholds["nsfw_media"] or \
                   result.get("nudity", {}).get("suggestive", 0) > thresholds["nsfw_media"]:
                    return {"nsfw_media"}
                if result.get("gore", {}).get("prob", 0) > thresholds["gore_violence_media"]: return {"gore_violence_media"}
                if result.get("offensive", {}).get("prob", 0) > thresholds["offensive_symbols_media"]: return {"offensive_symbols_media"}
    ...
    async def add_to_review_queue(self, message: discord.Message, reason: str):
        ...
        try:
            async with db_conn.cursor() as cursor:
                ...
                if cursor.rowcount > 0: await db_conn.commit()
                else: return
    ...
    @app_commands.command(name="clear_infractions", description="Clears ALL or a specific infraction for a user.")
    ...
    async def clear_infractions_command(self, interaction: discord.Interaction, member: discord.Member, infraction_id: int | None = None):
        if not interaction.guild_id or not interaction.user: return
        if infraction_id:
        ...
        if success:
            ...
            await interaction.response.send_message(f"Failed to clear {action_desc}. It may not exist.", ephemeral=True)
    ...
    async def _manual_action(self, interaction: discord.Interaction, user: discord.User, action_key: str, reason: str, duration: timedelta | None = None):
        ...
        if member and interaction.guild.owner_id != interaction.user.id and member.top_role >= interaction.user.top_role:
            await interaction.response.send_message("You cannot moderate a member with an equal or higher role.", ephemeral=True)
            return
        
        action_config = {"action": action_key, "reason_suffix": reason, "dm_message": f"You have been {action_key}ed by a moderator. Reason: {reason}"}
        if duration:
            action_config["duration_days"] = duration.days
            action_config["duration_hours"] = duration.total_seconds() / 3600

        if action_key in ["ban", "temp_ban"]:
            try:
                await interaction.guild.ban(user, reason=f"Manual action by {interaction.user.name}: {reason}", delete_message_seconds=0)
                if action_key == "temp_ban" and duration:
                    if not db_conn: return
                    unban_time = datetime.now(timezone.utc) + duration
                    async with db_conn.cursor() as cursor:
                        await cursor.execute('INSERT OR REPLACE INTO temp_bans (user_id, guild_id, unban_time, ban_reason) VALUES (?, ?, ?, ?)',
                                             (user.id, interaction.guild_id, unban_time.isoformat(), reason))
                    await db_conn.commit()
                await log_moderation_action(action_key, user, reason, interaction.user, interaction.guild, discord.Color.dark_red())
            ...
            await interaction.response.send_message("Cannot apply this action, user is not in the server.", ephemeral=True)
    ...
    @app_commands.command(name="manual_ban", description="Manually ban a user (temporarily or permanently).")
    @app_commands.default_permissions(ban_members=True)
    @app_commands.describe(user="User to ban (ID if not in server).", reason="Reason for ban.", duration_days="Optional duration for a temp ban.")
    ...
    async def manual_unban_command(self, interaction: discord.Interaction, user_id: str, reason: str):
        ...
        try:
            ...
            await log_moderation_action("unban_manual", target_user, reason, interaction.user, interaction.guild, discord.Color.green())
...
class AddRuleModal(discord.ui.Modal, title="Add New Violation Rule"):
    ...
    rule_type = discord.ui.Select(placeholder="Select the rule type to add...", options=[
        ...
        discord.SelectOption(label="Forbidden Phrase", value="forbidden_phrase", description="A sequence of words not allowed."),
        discord.SelectOption(label="Forbidden Regex", value="forbidden_regex", description="A regex pattern to detect violations.")
    ])
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(f"⚠️ Rule already exists. Review item `{self.review_id}` closed.", ephemeral=True)
        except Exception as e:
...
class ReviewActionView(discord.ui.View):
    ...
    async def punish_and_add_rule(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not isinstance(self.member, discord.Member):
            await interaction.response.send_message("Cannot punish user, they may have left. You can still add the rule.", ephemeral=True)
        if "openai" in self.reason.lower() and isinstance(self.member, discord.Member):
             await process_infractions_and_punish(self.member, self.member.guild, ["openai_flagged_moderate"], self.content, self.message_url)
        await interaction.response.send_modal(AddRuleModal(review_id=self.review_id))
    ...
    async def punish_only(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not isinstance(self.member, discord.Member):
            await interaction.response.send_message("Cannot punish user, they may have left.", ephemeral=True)
            return
        if "openai" in self.reason.lower():
            violation_type = "openai_flagged_moderate"
            await process_infractions_and_punish(self.member, self.member.guild, [violation_type], self.content, self.message_url)
            await self._close_review_item(interaction, f"✅ User punished for `{violation_type}` and review item `{self.review_id}` closed.", discord.Color.orange(), "Punished based on AI flag.")
        else:
            await interaction.response.send_message("This action is for AI flags. Use 'Punish & Add Rule' for others.", ephemeral=True)
    ...
    async def reject_as_safe(self, interaction: discord.Interaction, button: discord.ui.Button):
        logger.info(f"Review item {self.review_id} marked as safe by {interaction.user.name}. Content: '{self.content[:100]}...' Reason: {self.reason}")
        await self._close_review_item(interaction, f"✅ Review item `{self.review_id}` marked as safe.", discord.Color.green(), "Moderator marked as safe.")
